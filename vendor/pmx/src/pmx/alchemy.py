...
def _set_conformation(old_res, new_res, rotdic):
    ...
    for d in chis:
        ...
        diff = phi-phi2
        anchor_atoms = new_res.fetchm(d[1:3])
        if len(anchor_atoms) != 2:
            continue
        a1, a2 = anchor_atoms
        key = a1.name+'-'+a2.name
        if key not in rotdic:
            continue
        atoms = rotdic[key]
...
def _fetch_first_atom(residue, name):
    atoms = residue.fetch(name)
    if not atoms:
        return None
    return atoms[0]


def _resolve_dihedral_atoms(residue, atom_names):
    atoms = []
    for name in atom_names:
        target_residue = residue
        atom_name = name
        if name.startswith('+'):
            target_residue = residue.chain.fetch_residue(idx=residue.id + 1)
            atom_name = name[1:]
        ...
        if target_residue is None:
            return None
        atom = _fetch_first_atom(target_residue, atom_name)
        if atom is None:
            return None
        atoms.append(atom)
    return atoms
...
def _check_dih_ILDN_OPLS(topol, rlist, rdic, a1, a2, a3, a4):
    counter = 0
    for r in rlist:
        if getattr(a2, 'molecule', None) is r or a2 in getattr(r, 'atoms', []) or r.id == a2.resnr:
            dih = rdic[r.resname][3]
            for d in dih:
                if counter == 1:
                    break
                al = _resolve_dihedral_atoms(r, d[:4])
                if al is None:
                    continue
...
def _get_hybrid_residues(m, ff, version='new', verbose=False):
    ...
    for res in m.residues:
        if res.is_hybrid():
            ...
            for atom in hybrid_res.atoms:
                if atom.name not in atoms_by_name:
                    continue
                target = atoms_by_name[atom.name]
                target.atomtypeB = atom.atomtypeB
                target.qB = atom.qB
                target.mB = atom.mB
    return rlist, rdic
