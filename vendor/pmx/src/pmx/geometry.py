...
def bb_super(mol1, mol2, use_orig_mc_coords=True):
    ...
    if use_orig_mc_coords:
        ...
        atom_pairs = _common_named_atoms(atoms1, atoms2, [atom.name for atom in atoms2])
        assert len(atom_pairs) > 0, "%s -> %s" % ('-'.join(map(lambda a: a.name, atoms1)), '-'.join(map(lambda a: a.name, atoms2)))
        for atom1, atom2 in atom_pairs:
            atom2.x = atom1.x


def _common_named_atoms(atoms1, atoms2, ordered_names):
    atoms1_by_name = {atom.name: atom for atom in atoms1}
    atoms2_by_name = {atom.name: atom for atom in atoms2}
    return [
        (atoms1_by_name[name], atoms2_by_name[name])
        for name in ordered_names
        if name in atoms1_by_name and name in atoms2_by_name
    ]
