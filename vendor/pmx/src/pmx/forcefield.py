...
def _parse_topology_resnr(token):
    """Preserve insertion-coded residue labels such as ``52A``."""
...
def _format_topology_resnr(resnr):
    return str(resnr).strip()
...
class TopolBase:
    ...
    def __atom_from_top_line(self, line):
        ...
        idx = int(entr[0])
        atomtype = entr[1]
        resnr = _parse_topology_resnr(entr[2])
        resname = entr[3]
        name = entr[4]
        cgnr = int(entr[5])
        q = float(entr[6])
    ...
    def write_atoms(self, fp, charges='AB', atomtypes='AB', dummy_qA='on',
        ...
        al = self.atoms
        for atom in al:
            resnr = _format_topology_resnr(atom.resnr)
            if _atoms_morphe([atom]):
                ...
                if hasattr(atom, "qqB"):
                    ...
                    qqB = atom.qB
                print(
                    f"{atom.id:6d} {atA:>11s}{resnr:>7s}{atom.resname:>7s}{atom.name:>7s}"
                    f"{atom.cgnr:7d} {qqA:11.{qPrec}f}{mA:11.4f} {atB:>11s}"
                    f" {qqB:11.{qPrec}f}{mB:11.4f}",
                    file=fp,
                )
                self.qA += qqA
                self.qB += qqB
                ...
                print(
                    f"{atom.id:6d} {atom.atomtype:>11s}{resnr:>7s}{atom.resname:>7s}{atom.name:>7s}"
                    f"{atom.cgnr:7d} {atom.q:11.{qPrec}f}{atom.m:11.4f}",
                    file=fp,
                )
                self.qA += atom.q
                self.qB += atom.q
