...
if [ "${ABAG_ENABLE_TARGETED_REPEAT_SPREAD_RESCUE_WATCHER:-0}" != "1" ]; then
  echo "Targeted repeat-spread rescue watcher is disabled by default. Set ABAG_ENABLE_TARGETED_REPEAT_SPREAD_RESCUE_WATCHER=1 to enable it." >&2
...
ABAG_RBFE_EQUILIBRATION_SEED_PLAN_ROOTS="${ABAG_RBFE_EQUILIBRATION_SEED_PLAN_ROOTS:-${RESCUE_RUNS_ROOT}:${ROBUST_RUNS_ROOT}:${PRIORITY_RUNS_ROOT}}"
...
MERGED_EXTRA_PLAN_ROOT_3="${MERGED_EXTRA_PLAN_ROOT_3:-${RUNS_ROOT}}"
...
REFRESH_TARGETED_RESCUES_EACH_PASS="${REFRESH_TARGETED_RESCUES_EACH_PASS:-1}"
WATCHLIST_REFRESH_JSON="${WATCHLIST_REFRESH_JSON:-${RUNS_ROOT}/reports/watch/validation_targeted_repeat_spread_watchlist_refresh.json}"
POST_REFRESH_COMMAND="${POST_REFRESH_COMMAND:-${VALIDATION_POST_REPORT_REFRESH}}"
TARGETED_NO_ACTIVE_ALT_ABS_ERROR_THRESHOLD="${TARGETED_NO_ACTIVE_ALT_ABS_ERROR_THRESHOLD:-3.0}"
...
refresh_job_ids() {
  ...
  mapfile -t refreshed < <(
    python3 -u "${BENCHMARK_ROOT}/refresh_validation_watchlists.py" \
      ...
      --targeted-no-active-alt-abs-error-threshold "${TARGETED_NO_ACTIVE_ALT_ABS_ERROR_THRESHOLD}" \
...
run_watch_pass() {
  ...
  if [ "${REFRESH_TARGETED_RESCUES_EACH_PASS}" = "1" ]; then
    "${ROOT}/benchmarks/ab_bind/refresh_validation_targeted_repeat_spread_rescues.sh" "${JOB_IDS[@]}" >/dev/null
  fi
