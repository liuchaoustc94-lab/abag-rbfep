...
ABAG_RBFE="${ABAG_RBFE:-${ROOT}/.venv/bin/abag-rbfe}"
BENCHMARK_ROOT="${BENCHMARK_ROOT:-${ROOT}/benchmarks/ab_bind}"
SPLIT_FILE="${SPLIT_FILE:-${BENCHMARK_ROOT}/splits/ab_bind_rbfe_core_v1_split_v1.yml}"
RUNS_ROOT="${RUNS_ROOT:-${ROOT}/runs/benchmarks/abbind_core_v1_validation_sampling_qc_rescues}"
PRIORITY_RUNS_ROOT="${PRIORITY_RUNS_ROOT:-${ROOT}/runs/benchmarks/abbind_core_v1_validation_priority_plan}"
ROBUST_RUNS_ROOT="${ROBUST_RUNS_ROOT:-${ROOT}/runs/benchmarks/abbind_core_v1_validation_robust_plan}"
RESCUE_RUNS_ROOT="${RESCUE_RUNS_ROOT:-${ROOT}/runs/benchmarks/abbind_core_v1_validation_priority_rescues}"
TARGETED_REPEAT_SPREAD_RESCUE_RUNS_ROOT="${TARGETED_REPEAT_SPREAD_RESCUE_RUNS_ROOT:-${ROOT}/runs/benchmarks/abbind_core_v1_validation_targeted_repeat_spread_rescues}"
...
DEEP_RESCUE_RUNS_ROOT="${DEEP_RESCUE_RUNS_ROOT:-${ROOT}/runs/benchmarks/abbind_core_v1_validation_deep_rescues}"
ULTRA_RESCUE_RUNS_ROOT="${ULTRA_RESCUE_RUNS_ROOT:-${ROOT}/runs/benchmarks/abbind_core_v1_validation_ultra_rescues}"
...
if [ "${ABAG_ENABLE_SAMPLING_QC_RESCUE_WATCHER:-0}" != "1" ]; then
  echo "Sampling/QC rescue watcher is disabled by default. Set ABAG_ENABLE_SAMPLING_QC_RESCUE_WATCHER=1 to enable it." >&2
...
SAMPLING_QC_COMPLEX_IDS="${SAMPLING_QC_COMPLEX_IDS:-}"
ABAG_RBFE_EQUILIBRATION_SEED_PLAN_ROOTS="${ABAG_RBFE_EQUILIBRATION_SEED_PLAN_ROOTS:-${RESCUE_RUNS_ROOT}:${TARGETED_REPEAT_SPREAD_RESCUE_RUNS_ROOT}:${ROBUST_RUNS_ROOT}:${PRIORITY_RUNS_ROOT}}"
...
REFRESH_SAMPLING_QC_RESCUES_EACH_PASS="${REFRESH_SAMPLING_QC_RESCUES_EACH_PASS:-1}"
WATCHLIST_REFRESH_JSON="${WATCHLIST_REFRESH_JSON:-${RUNS_ROOT}/reports/watch/validation_sampling_qc_watchlist_refresh.json}"
POST_REFRESH_COMMAND="${POST_REFRESH_COMMAND:-${VALIDATION_POST_REPORT_REFRESH}}"
SAMPLING_QC_NO_ACTIVE_ALT_ABS_ERROR_THRESHOLD="${SAMPLING_QC_NO_ACTIVE_ALT_ABS_ERROR_THRESHOLD:-2.5}"
...
refresh_job_ids() {
  local refresh_args=(
    python3
    -u
    "${BENCHMARK_ROOT}/refresh_validation_watchlists.py"
    --root "${ROOT}"
    --mode sampling_qc
    --robust-plan-root "${ROBUST_RUNS_ROOT}"
    --rescue-plan-root "${RESCUE_RUNS_ROOT}"
    --targeted-repeat-spread-plan-root "${TARGETED_REPEAT_SPREAD_RESCUE_RUNS_ROOT}"
    --sampling-qc-plan-root "${RUNS_ROOT}"
    --deep-rescue-plan-root "${DEEP_RESCUE_RUNS_ROOT}"
    --ultra-rescue-plan-root "${ULTRA_RESCUE_RUNS_ROOT}"
    --sampling-qc-no-active-alt-abs-error-threshold "${SAMPLING_QC_NO_ACTIVE_ALT_ABS_ERROR_THRESHOLD}"
    --output-json "${WATCHLIST_REFRESH_JSON}"
  )

  if [ -n "${SAMPLING_QC_COMPLEX_IDS}" ]; then
    IFS=',' read -r -a SAMPLING_QC_COMPLEX_ID_VALUES <<< "${SAMPLING_QC_COMPLEX_IDS}"
    local complex_id
    for complex_id in "${SAMPLING_QC_COMPLEX_ID_VALUES[@]}"; do
      complex_id="${complex_id//[[:space:]]/}"
      if [ -n "${complex_id}" ]; then
        refresh_args+=(--sampling-qc-complex-id "${complex_id}")
      fi
    done
  fi

  local refreshed=()
  mapfile -t refreshed < <("${refresh_args[@]}")
  if [ "${#refreshed[@]}" -gt 0 ]; then
    JOB_IDS=("${refreshed[@]}")
  fi
}

ensure_sampling_qc_plan_reports() {
  local force_refresh="${1:-0}"
  local plan_jobs_csv="${RUNS_ROOT}/reports/plan_jobs.csv"
  if [ "${force_refresh}" != "1" ] && [ -f "${plan_jobs_csv}" ]; then
    return
  fi
  "${ABAG_RBFE}" \
    batch \
    ...
    --split-file "${SPLIT_FILE}" >/dev/null
}
...
run_watch_pass() {
  ...
  if [ "${REFRESH_SAMPLING_QC_RESCUES_EACH_PASS}" = "1" ]; then
    "${ROOT}/benchmarks/ab_bind/refresh_validation_sampling_qc_rescues.sh" "${JOB_IDS[@]}" >/dev/null
    ensure_sampling_qc_plan_reports 1
  else
    ensure_sampling_qc_plan_reports 0
  fi
