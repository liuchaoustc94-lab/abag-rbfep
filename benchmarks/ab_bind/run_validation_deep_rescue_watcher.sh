...
if [ "${ABAG_ENABLE_DEEP_RESCUE_WATCHER:-0}" != "1" ]; then
  echo "Deep rescue watcher is disabled by default. Set ABAG_ENABLE_DEEP_RESCUE_WATCHER=1 to enable it." >&2
...
# Deep rescues should backfill around baseline validation, not starve it.
...
REFRESH_DEEP_RESCUES_EACH_PASS="${REFRESH_DEEP_RESCUES_EACH_PASS:-1}"
...
run_watch_pass() {
  ...
  if [ "${REFRESH_DEEP_RESCUES_EACH_PASS}" = "1" ]; then
    "${ROOT}/benchmarks/ab_bind/refresh_validation_deep_rescues.sh" "${JOB_IDS[@]}" >/dev/null
  fi
