...
DEEP_PLAN_ROOT="${ROOT}/runs/benchmarks/abbind_core_v1_validation_deep_rescues"
RUNS_ROOT="${ROOT}/runs/benchmarks/abbind_core_v1_validation_ultra_rescues"
BATCH_PREFIX="${BATCH_PREFIX:-abbind-ultra-rescue}"
REPEAT_INCREMENT="${REPEAT_INCREMENT:-2}"
LAMBDA_INCREMENT="${LAMBDA_INCREMENT:-8}"
...
REQUIRE_ACTIVE_ALTERNATE="${REQUIRE_ACTIVE_ALTERNATE:-1}"
ALLOW_PASS_QC_OUTLIER_RESCUE="${ALLOW_PASS_QC_OUTLIER_RESCUE:-0}"
HOTSPOT_COMPLEX_IDS="${HOTSPOT_COMPLEX_IDS:-3HFM}"
HOTSPOT_REPEAT_INCREMENT="${HOTSPOT_REPEAT_INCREMENT:-3}"
HOTSPOT_LAMBDA_INCREMENT="${HOTSPOT_LAMBDA_INCREMENT:-12}"
HOTSPOT_PRODUCTION_SCALE="${HOTSPOT_PRODUCTION_SCALE:-3.0}"
...
CMD=(
  ...
  --extra-plan-root "${DEEP_PLAN_ROOT}"
...
if [ "${ALLOW_PASS_QC_OUTLIER_RESCUE}" = "1" ]; then
  CMD+=(--allow-pass-qc-outlier-rescue)
fi
