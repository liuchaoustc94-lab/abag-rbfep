...
RUNS_ROOT="${ROOT}/runs/benchmarks/abbind_core_v1_validation_priority_rescues"
...
if [ "${ABAG_ENABLE_RESCUE_WATCHER:-0}" != "1" ]; then
  echo "Rescue watcher is disabled by default. Use run_validation_watchers.sh start rescue|all or set ABAG_ENABLE_RESCUE_WATCHER=1." >&2
...
ABAG_RBFE_EQUILIBRATION_SEED_PLAN_ROOTS="${ABAG_RBFE_EQUILIBRATION_SEED_PLAN_ROOTS:-${PRIORITY_RUNS_ROOT}:${ROBUST_RUNS_ROOT}:${TARGETED_REPEAT_SPREAD_RESCUE_RUNS_ROOT}}"
MERGED_PLAN_ROOT="${MERGED_PLAN_ROOT:-${RUNS_ROOT}}"
MERGED_EXTRA_PLAN_ROOT_1="${MERGED_EXTRA_PLAN_ROOT_1:-${PRIORITY_RUNS_ROOT}}"
MERGED_EXTRA_PLAN_ROOT_2="${MERGED_EXTRA_PLAN_ROOT_2:-${ROBUST_RUNS_ROOT}}"
...
run_watch_pass() {
  ...
  if [ "${REFRESH_RESCUES_EACH_PASS}" = "1" ]; then
    "${ROOT}/benchmarks/ab_bind/refresh_validation_rescues.sh" "${JOB_IDS[@]}" >/dev/null
  fi
