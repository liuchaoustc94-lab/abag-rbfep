...
## 4.13 新协议 Phase B + 扩展验证完整判读（2026-08-12）

51 jobs 并行池（3 GPU × 2 并发）~11 小时跑完，42 立即就绪 + 10 失败重试中（dt 0.001）。

### ① B1 自适应 λ：QC 目标达成，但误差非采样驱动（重要阴性结果）
| job | overlap | 误差 |
|---|---|---|
| 6 个芳香删除 job | 0.08-0.26 → **0.23-0.36（全部≥0.2 ✓）** | MAE 7.10 → 7.41（未改善） |
**结论：16λ + 分步调度确实解决了 overlap；但芳香删除的残差不随采样改善——是系统性（力场/端点效应），不再投入更多采样。**

### ② B2 分步调度：overlap 崩塌修复，纯态模型否证依旧成立
- complex 腿 overlap 0.064 → **0.211 ✓**（分步调度对电荷变化转化完全有效，V2.1 前置达成）
- ddG 仍 −21.6（baseline 中性 −0.02，exp +1.88）——再次确认纯 HIP 态模型对 74% 中间态物理上错误；**H33 的正确做法是中性默认 + 形式化校正（+1.3 kcal/mol）→ 预测 ≈ +1.3 vs exp +1.88，相当好**。

### ③ C2 三个新靶点样本外验证（n=28）
| 靶点 | n | R | MAE | sign |
|---|---|---|---|---|
| 1AK4（CycA/HIV 衣壳） | 9 | 0.471 | 1.55 | 0.78 |
| 1KTZ（TGF-β R/配体） | 14 | 0.416 | 1.37 | 0.71 |
| 3K2M（Monobody/SH2） | 5 | 0.299* | 2.35 | 1.00 |
| **合并** | **28** | **0.418** | **1.60** | **0.79** |
*3K2M 的 4.0×3 为截断值（被 censored 规则捕获）。加上 1DVF 17 点，样本外验证集达 **n=45**。

### ④ 新发现问题：dt=0.002 在柔性体系不稳定
- 7 个 job（6 个 1AK4 CypA 结合环区 + 1vfb-g31a）sample 阶段 SIGSEGV（LINCS 警告后崩溃），均集中在高柔性区域；2jel 同类 exit 1。
- **策略：dt=0.002 保留为默认（成本减半），失败自动回退 dt=0.001 重试**（已在重试队列验证此路径）。后续可在 sample 阶段加入自动 dt 减半回退机制。

### ⑤ 3BE1-y33a prepare 拦截：真实输入缺陷
GLU A598 vs ARG L27D CD-CZ 1.131 Å（物理不可能的侧链重叠，AB-Bind 结构库缺陷）。注意 prepare 的 inter-residue 拦截目前不做分层（任何 inter clash 都 block，而 intra 有侧链可修复路径）——已记录为待改进点。fit pairs 暂以 10/11 计。

## 4.14 Basin 实验 + validation 面板中期结果（2026-08-13）

### ① Basin 多样性实验（裁决 RID）——触发预设门槛
- y102a：3 basin 的 ddG = [13.37, 12.43, 15.11]，**spread 2.67**
- y49a：[−7.33, −5.05, −8.39]，**spread 3.34**
- 均 >1 kcal/mol 预设门槛 → **basin 多样性是真实误差源，RID/系综方法获批入场**。但注意 basin 均值（13.6 / −6.9）仍远离实验（4.8 / 1.9）——basin 只解释方差部分，均值的系统性偏差另有来源（力场/水合）。

### ② Validation 面板（37 pairs，化学完整+新协议，raw）的诚实分层
| 口径 | R | MAE |
|---|---|---|
| 全部 37 | 0.523 | 2.39 |
| 剔除 5 个 censored 饱和点 | **0.169** | 1.78 |
| 强效应分类（exp≥2.5）AUC | **0.833** | — |
| 强效应点（n=6） | — | 5.84（高估，如 16-17 vs 4-5） |
| 弱效应点（n=31） | — | 1.72 |

**关键认识：表观 R=0.52 主要由热点区分驱动；弱效应排序仍接近噪声（1BJ1 剔除后甚至 −0.72）。模型的真实能力画像 = 能分热点（AUC 0.83）+ 强效应幅值高估 + 弱效应不可排序。** 这比单一 R 数字诚实得多，也指明了改进方向（动态范围压缩 = 力场/水合问题）。

## 4.15 三引擎对照 + 方向感知收口 + soft-core A/B（2026-08-17）

### ① RID basin-FEP 裁决（预设规则：误差减半=端点系综成立）
| job | 标准 | 自由MD basin | **RID basin** | 实验 |
|---|---|---|---|---|
| y102a | 14.59 | 13.63 | **10.04**（3 basin 均值） | 4.79 |
| y49a | −7.75 | −6.92 | **−0.84** | 1.90 |

误差削减：y102a −46%（接近减半）、y49a **−72%**。**裁决：端点系综策略成立（部分）**——RID basin 显著向实验值移动，但均值仍有残差（力场/水合层）。REST2 臂的 seed-FEP 正在跑（补齐三引擎对照）。

### ② 方向感知调度收口：7 个插入 job 救回 5 个
- 救回：g489v/a488v/g489a/a492v/s52l（误差 1.2-2.8，正常水平）
- 残余 2 个的机理不同：p490v（P→V，净重原子差=0 逃过方向启发式→**Pro 规则已加**：Pro 相关突变一律按插入调度）+ h487q（**不同的错误**：excluded perturbed pairs 超 rlist 的 GROMACS FEP fatal → 新增 `couple_intramol` protocol 开关处置）
- 1KTZ 救回后 R=0.416→**0.751**；1AK4 R=0.471→0.578；C2 合并 R=0.418→**0.502**（n=33）

### ③ Gapsys soft-core A/B：阴性结果，保持 Beutler 默认
- q89a：overlap 1.000 但 ddG −6.12（Beutler −3.94，exp 1.75）变差
- g88a：overlap_min=0.000（直接坏掉）
- **裁决：sc_alpha=0 在我们的 GROMACS 2027-dev + pmx 拓扑组合下不是即插即用的改进，保持默认**（已记录，不切换）

## 4.16 三引擎对照完成 + 收尾状态（2026-08-18）

### ① REST2 seed-FEP 完成——三引擎对照终局
| job | 标准 | 自由MD | RID basin | **REST2 seed** | 实验 |
|---|---|---|---|---|---|
| y102a | 14.59 | 13.63 | **10.04** | 11.50 | 4.79 |
| y49a | −7.75 | −6.92 | **−0.84** | **−0.29** | 1.90 |

**裁决**：RID 与 REST2 端点系综都把 y49a 从灾难级拉回实验附近（err 9.7→~2.5，-75%）；y102a 的均值也只被部分拉动（err 减半）。**端点系综策略正式确立为 Y/W 删除类的标准处理流程**（RID 略优于 REST2；自由 MD 单轨迹不足以覆盖 basin）。

### ② 顽固 job 最终状态
- p490v（P→V）：**Pro 规则救回**（ddG=8.66 vs exp 2.00——值不佳但流程闭环；该环区本身就是最难案例）
- h487q：couple_intramol=yes 后仍失败——pre_relax λ005 处 Coulomb(SR)=−8.8×10⁶ kJ/mol 电灾（紧密环区中间态电冲突），已用 24λ 加密 + 2ps relax 做最后一试

### ③ fit 面板尾部 30 个未完成 job 分类
17 sample 失败（8 SIGSEGV + 9 exit1）、6 prepare blocked（输入缺陷）、6 mutate 失败/拦截、1 equilibrate——待 fit 主队列结束后统一 triage（多数 sample 失败可能是方向感知/Pro 规则上线前的旧尝试）。

## 4.17 PR-4 首轮判读与 PR-4b 启动（2026-08-19）

### PR-4 首轮（24λ/20ps 耦合调度）结果：机械成立，协议欠弛豫
| 突变 | exp | 我们 | Patel 1ns | Patel 收敛(8-10ns) |
|---|---|---|---|---|
| n31e | 5.71 | **6.69 (err 0.98)** | 13.28 | 3.52 |
| d101k | 2.13 | 4.19 (2.06) | 16.98 | 0.12 |
| r21a | 0.90 | 9.94 (9.04) | 5.82 | 1.35 |
| k97d | 6.77 | 39.12 (32.35) | 10.25 | 8.24 |

- MAE：我们 11.1 vs Patel-1ns 7.7；**剔除 k97d：我们 4.0 vs Patel 7.7（同采样量更优）**
- k97d QC 全干净（杂化✓、电荷不变性 δ=0 ✓、repeat 紧一致 36.8-40.7）——**统计收敛但物理未弛豫**：电荷翻转的水/离子重组是 ns 级慢过程，20ps/窗口处于未弛豫区
- Patel 自己的 1ns 同样全错（Table 2 实证收敛需 5-8ns transition）→ 与我们 E4 结论互证：端点/弛豫深度决定收敛

### PR-4b（已启动 2026-08-19）
5 突变按 E4 模式重跑：**40ns 自由 MD 端点系综 + 3 帧 seed（12.5/25/37.5ns）+ production_ps 20→50**，耦合 24λ。这是与 Patel R²=0.81 的公平对照协议。脚本：`run_pr4b_charge_ensemble_20260819.sh`。预计 ~1 天。
